...
```bash
uv run python - <<'PY'
from nowpy_mcp.server import test_connection
...
```bash
uv run python - <<'PY'
from nowpy_mcp.server import get_records
...
```bash
uv run python - <<'PY'
from nowpy_mcp.server import create_script_include
...
```bash
uv run python - <<'PY'
from nowpy_mcp.server import create_catalog_item_bundle
...
```bash
uv run python - <<'PY'
from nowpy_mcp.server import create_atf_test_bundle
...
```bash
uv run python - <<'PY'
from nowpy_mcp.server import update_catalog_item
...
```bash
uv run python - <<'PY'
from nowpy_mcp.server import delete_record
...
```bash
uv run mcp dev src/nowpy_mcp/server.py
```
