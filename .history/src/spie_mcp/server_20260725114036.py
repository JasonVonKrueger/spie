...
def _script_include_script_text(row: dict[str, Any]) -> str:
    script_value = row.get("script", "")
    if isinstance(script_value, dict):
        script_value = script_value.get("value", "")
    return str(script_value or "")


def _normalize_script_text(script_text: str) -> str:
    without_block_comments = re.sub(r"/\*.*?\*/", "", script_text, flags=re.S)
    without_line_comments = re.sub(r"//.*?$", "", without_block_comments, flags=re.M)
    compact = re.sub(r"\s+", " ", without_line_comments)
    return compact.strip().lower()


def _script_include_signatures(script_text: str) -> list[str]:
    signatures: list[str] = []
    class_match = re.search(r"var\s+([A-Za-z0-9_]+)\s*=\s*Class\.create\(\)", script_text)
    if class_match:
        signatures.append(f"class:{class_match.group(1)}")

    for match in re.finditer(r"function\s+([A-Za-z0-9_]+)\s*\(", script_text):
        signatures.append(f"function:{match.group(1)}")

    for match in re.finditer(r"([A-Za-z0-9_]+)\s*:\s*function\s*\(", script_text):
        signatures.append(f"method:{match.group(1)}")

    return signatures


def _script_include_function_blocks(script_text: str) -> list[dict[str, str]]:
    blocks: list[dict[str, str]] = []
    patterns = [
        ("function", re.compile(r"function\s+([A-Za-z0-9_]+)\s*\([^)]*\)\s*{")),
        ("method", re.compile(r"([A-Za-z0-9_]+)\s*:\s*function\s*\([^)]*\)\s*{")),
    ]
            ...
            blocks.append(
                {
                    "kind": kind,
                    "name": match.group(1),
                    "body": script_text[match.start() : end],
                }
            )
...
def _normalize_function_body(function_text: str) -> str:
    without_block_comments = re.sub(r"/\*.*?\*/", "", function_text, flags=re.S)
    without_line_comments = re.sub(r"//.*?$", "", without_block_comments, flags=re.M)
    without_strings = re.sub(r"(['\"])(?:\\.|(?!\1).)*\1", "STR", without_line_comments)
    without_numbers = re.sub(r"\b\d+(?:\.\d+)?\b", "NUM", without_strings)
    tokens = re.findall(r"[A-Za-z_][A-Za-z0-9_]*|==|!=|===|!==|<=|>=|&&|\|\||[{}()[\].,:;+-/*%<>!=]", without_numbers.lower())
    keywords = {
        "as",
        "break",
        "case",
    ...
    normalized_tokens = [token if not re.fullmatch(r"[a-z_][a-z0-9_]*", token) or token in keywords else "id" for token in tokens]
    return " ".join(normalized_tokens).strip()


def _function_similarity(left_body: str, right_body: str) -> float:
...
def _chunk_script_includes(records: list[dict[str, Any]], chunk_size: int = 200) -> list[list[dict[str, Any]]]:
    ...
    for index in range(0, len(records), chunk_size):
        chunks.append(records[index : index + chunk_size])
    return chunks


def _fetch_script_includes(sample_limit: int = 0) -> list[dict[str, Any]]:
    client = _client()
    ...
    while True:
        response = client.get_records(
            "sys_script_include",
            fields=["sys_id", "name", "api_name", "description", "script", "active", "client_callable"],
            limit=page_size,
            offset=offset,
            display_value="all",
        )
...
def _group_by_key(records: list[dict[str, Any]], key_fn: Any) -> list[list[dict[str, Any]]]:
...
def _script_include_redundancy_scan(records: list[dict[str, Any]]) -> dict[str, Any]:
    normalized_groups = _group_by_key(records, lambda row: _normalize_script_text(_script_include_script_text(row)))
    api_name_groups = _group_by_key(records, lambda row: _string_value(row.get("api_name")))
    signature_groups = _group_by_key(
        records,
        lambda row: "|".join(sorted(set(_script_include_signatures(_script_include_script_text(row))))),
    )
    ...
    function_similarity_groups: list[dict[str, Any]] = []
    seen_pairs: set[tuple[str, str, str, str]] = set()
    for index, left in enumerate(function_entries):
        for right in function_entries[index + 1 :]:
            ...
            pair_key = (
                left["include"]["sys_id"],
                left["function_name"],
                right["include"]["sys_id"],
                right["function_name"],
            )
            if pair_key in seen_pairs:
                continue
            seen_pairs.add(pair_key)
            function_similarity_groups.append(
                {
    ...
    likely_redundant_pairs: list[dict[str, Any]] = []
    for group in normalized_groups:
        likely_redundant_pairs.append(
            {
                "reason": "identical_script_body",
                "includes": [
                    {
                        "sys_id": _string_value(row.get("sys_id")),
                        "name": _string_value(row.get("name")),
    ...
    for group in signature_groups:
        likely_redundant_pairs.append(
            {
                "reason": "shared_helper_or_class_signatures",
                "includes": [
                    {
                        "sys_id": _string_value(row.get("sys_id")),
                        "name": _string_value(row.get("name")),
    ...
    for match in function_similarity_groups:
        likely_redundant_pairs.append(
            {
                "reason": "similar_function_body",
    ...
    return {
        "total_script_includes": len(records),
        ...
        "redundant_function_groups": function_similarity_groups,
        "likely_redundant_pairs": likely_redundant_pairs,
        "recommendations": [
...
def _script_include_naming_assessment(name: str) -> dict[str, Any]:
    normalized = _normalized_phrase(name)
    passed = bool(re.fullmatch(r"[A-Z][A-Za-z0-9]+(?:Helper|Service|Validator)", normalized))
    detail = "Script Include names should follow CompanyPrefixFeatureHelper/Service/Validator naming."
    if passed:
        detail = "Script Include naming matches the documented convention."
    return {
        "name": normalized,
        "passed": passed,
        "detail": detail,
...
def _business_rule_naming_assessment(name: str) -> dict[str, Any]:
    normalized = _normalized_phrase(name)
    passed = normalized.count(" - ") >= 2
    detail = "Business Rule names should follow '<Table> - <Trigger or Outcome> - <Short Purpose>'."
    if passed:
        detail = "Business Rule naming matches the documented convention."
    return {
        "name": normalized,
        "passed": passed,
        "detail": detail,
...
def _client_script_naming_assessment(name: str) -> dict[str, Any]:
    normalized = _normalized_phrase(name)
    passed = normalized.count(" - ") >= 2
    detail = "Client Script names should follow '<Table or Item> - <Behavior> - <Purpose>'."
    if passed:
        detail = "Client Script naming matches the documented convention."
    return {
        "name": normalized,
        "passed": passed,
        "detail": detail,
...
def _validated_script_include_name(name: str) -> str:
    assessment = _script_include_naming_assessment(name)
    if not assessment["passed"]:
        raise ValueError(assessment["detail"])
    return str(assessment["name"])


def _validated_business_rule_name(name: str) -> str:
    assessment = _business_rule_naming_assessment(name)
    if not assessment["passed"]:
        raise ValueError(assessment["detail"])
    return str(assessment["name"])


def _validated_client_script_name(name: str) -> str:
    assessment = _client_script_naming_assessment(name)
    if not assessment["passed"]:
        raise ValueError(assessment["detail"])
    return str(assessment["name"])
...
def create_update_set(
    ...
    result = _client().create_record("sys_update_set", payload, allow_protected_tables=True)
    return {
...
def update_update_set(
    ...
    """Update a ServiceNow update set, enforcing naming convention checks for renames."""
    payload: dict[str, Any] = {}
    naming_validation: dict[str, Any] | None = None
    ...
    result = _client().update_record(
        "sys_update_set",
        sys_id,
        payload,
        allow_protected_tables=True,
    )
    response = dict(result)
    if naming_validation is not None:
        response["naming_validation"] = naming_validation
    return response
...
def create_script_include(
    ...
    validated_name = _validated_script_include_name(name)
    ...
    result = _client().create_record("sys_script_include", payload, allow_protected_tables=True)
    return {
        "naming_validation": _script_include_naming_assessment(validated_name),
        **result,
    }
...
def create_business_rule(
    ...
    validated_name = _validated_business_rule_name(name)
    payload = _clean_payload(
        {
            "name": validated_name,
            "collection": table,
    ...
    result = _client().create_record("sys_script", payload, allow_protected_tables=True)
    return {
        "naming_validation": _business_rule_naming_assessment(validated_name),
        **result,
    }
...
def create_client_script(
    ...
    validated_name = _validated_client_script_name(name)
    payload = _clean_payload(
        {
            "name": validated_name,
            "table": table,
    ...
    result = _client().create_record("sys_script_client", payload, allow_protected_tables=True)
    return {
        "naming_validation": _client_script_naming_assessment(validated_name),
        **result,
    }
...
def create_catalog_client_script(
    ...
    result = _client().create_record("catalog_script_client", payload, allow_protected_tables=True)
    return {
        "naming_validation": _client_script_naming_assessment(validated_name),
        **result,
    }
...
def create_atf_test_step(
    ...
    try:
        ...
        raise ValueError(
            "ATF step input value creation failed after step creation. "
            f"Manual cleanup may be required for sys_atf_step={step_sys_id}. Original error: {exc}"
        ) from exc
...
def create_atf_test_bundle(
    ...
    try:
        ...
        created_step_ids = []
        for created_step in created_steps:
            step_record = created_step.get("result", {})
            step_sys_id = _extract_value(step_record.get("sys_id"))
            if step_sys_id:
                created_step_ids.append(str(step_sys_id))
        raise ValueError(
            "ATF test bundle creation failed after partial creation. "
            f"Manual cleanup may be required for sys_atf_test={test_sys_id} and sys_atf_step records={created_step_ids}. "
...
def validate_permissions(
    ...
    """Validate read access and optional create access for target ServiceNow tables."""
    client = _client()
