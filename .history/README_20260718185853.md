...
The workspace already includes [.vscode/mcp.json](/Users/rogerpoore/nowpy/.vscode/mcp.json), so VS Code can launch the server as a local MCP process with:
...
```json
{
  "servers": {
	"servicenow-mcp": {
	  "type": "stdio",
	  "command": "uv",
	  "args": ["run", "python", "-m", "nowpy_servicenow_mcp"]
	}
  }
}
```
...
```bash
uv run python - <<'PY'
from nowpy_servicenow_mcp.server import test_connection
...
```bash
uv run python - <<'PY'
from nowpy_servicenow_mcp.server import get_records
...
```bash
uv run python - <<'PY'
from nowpy_servicenow_mcp.server import create_script_include
...
```bash
uv run python - <<'PY'
from nowpy_servicenow_mcp.server import create_catalog_item_bundle
...
```bash
uv run python - <<'PY'
from nowpy_servicenow_mcp.server import create_atf_test_bundle
...
```bash
uv run python - <<'PY'
from nowpy_servicenow_mcp.server import update_catalog_item
...
```bash
uv run python - <<'PY'
from nowpy_servicenow_mcp.server import delete_record
